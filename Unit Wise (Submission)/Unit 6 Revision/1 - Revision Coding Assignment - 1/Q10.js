...
const hRated = sciFiMovies.filter((m) => m.rating >= 8);

// Sort the filtered list by year (oldest → newest).
const sorted = hRated.sort((a, b) => a.year - b.year);

// Use reduce() to calculate the average rating of these filtered movies.
const avgR = (
  hRated.reduce((acc, m) => acc + m.rating, 0) / hRated.length
).toFixed(2);
...
console.log("Average Rating:", avgR);
